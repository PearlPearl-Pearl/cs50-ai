...
def get_mask_token_index(mask_token_id, inputs):
    ...
    if not all(mask_token_id in sequence for sequence in inputs['input_ids']):
        return None
    some_array = list(tf.reshape(inputs['input_ids'], [-1]))
    return some_array.index(mask_token_id)
