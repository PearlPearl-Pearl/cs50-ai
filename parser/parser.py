...
def preprocess(sentence):
    ...
    final_sentence = [word for word in tokenized_sentence if any(char.isalpha() for char in word)]
...
def np_chunk(tree):
    ...
    for item in tree.subtrees():
        ...
        if item.label() == "NP" and len(list(item.subtrees()))==1:
            chunks.append(item)

        if item.label() == "N":
            chunks.append(item.parent())
